...
def test_skeleton():
    ...
    assert_equal(len(s.segments),1)
    info = s.info()
    assert(info.startswith('N'))

def test_io():
    assert_is_instance(io, AmirameshReader)
    f = open('data/lariat.am')
    skel = io.parse(f)
    assert_equal(len(skel.nodes),2)
    ...
    assert_equal(len(skel.segments[0].points[0].diameters),1)
    assert_equal(len(skel.segments[1].points[0].diameters),1)
